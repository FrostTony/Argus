...
func parseFlags(name string, args []string) (*flags, error) {
	...
	if len(f.probes) == 0 {
		return nil, fmt.Errorf("-probes is required")
	}
	return f, nil
}

func load(f *flags) (config.Server, config.Probes, *slog.Logger, error) {
	srv, err := config.LoadServer(f.server)
	if err != nil {
		return srv, config.Probes{}, nil, err
	}
	if f.level != "" {
...
func cmdRun(args []string) error {
	...
	srvCfg, probesCfg, log, err := load(f)
	if err != nil {
		return err
	}
	a, err := app.Build(srvCfg, probesCfg, log)
	if err != nil {
		return err
	}
	defer a.Close()

	// Reloading re-reads the same files, so SIGHUP and the API behave alike.
	reload := func() error {
		probes, err := config.LoadProbes(f.probes...)
		if err != nil {
			return err
	...
	if every := srvCfg.Probing.WatchConfig.D(); every > 0 {
...
// Only a single file qualifies: a directory has no unambiguous destination.
...
func cmdValidate(args []string) error {
	...
	st := a.Status()
	idle, err := checkIdle(probesCfg, srvCfg.Node.Name, st)
	if err != nil {
		return err
	}
